...
{
    getline(f,linie); cout <<linie <<endl;
} f.close();
...
int ParseDirectory(string dir,string name,char s);
void citire(char in[256])
{
    ...
    else
    {
        ...
        else if(intrare[0].sir=="find")
        {
            ...
            {
                if(intrare[2].sir=="a")
                {ParseDirectory(intrare[3].sir+":/",intrare[4].sir,'a');


                }else if(intrare[2].sir=="b")
                {ParseDirectory(intrare[3].sir+":/",intrare[4].sir,'b');

               } else if(intrare[2].sir=="c") ParseDirectory(intrare[3].sir+":/",intrare[4].sir,'c');

            }
        }

        else if(intrare[0].sir=="cut"){ifstream f(intrare[1].sir.c_str());int nr=0;
...
int ParseDirectory(string dir,string name,char s)
{
    ...
    while( (dirP = readdir(tDir)) )
    {
        ...
        if(path==pathaux+name) //E:/boost/a.txt
        {
    ...
    } else if(s=='b')
    {
      ...
      int i= path.length(); int loc=-1,pus=1,ok=1;
   for(i=path.length()-1;i>=0;i--)
      {
          if(path[i]!='.'&&pus==1){loc++;pathaux2[loc]=path[i];}
          else if(path[i]=='.'&&ok==1){ pus=0;ok=0;}
          else if(path[i]=='/'){loc++;pathaux2[loc]=path[i];pus=1;}
...
for(int i=loc;i>=0;i--) // find -r b E ana
   ...
   //         cout <<pathaux2<<endl;
            ...
            if(pathaux2+name==pathaux)cout <<path<<endl;
    }
...
int pus=0,loc=-1,ok=1;
         ...
         {if(path[i]!='.'&&pus==1){loc++;pathaux2[loc]=path[i];}
         else if(path[i]=='.' &&ok==1){ok=1;pus=1;}
